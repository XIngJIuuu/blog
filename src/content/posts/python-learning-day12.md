...
description: "Python 第十二天笔记，按条目排的可查清单。主线只有一句：排序真正比较的不是元素本身，而是 key 函数产出的那串值——由此推出四件事。稳定性是唯一能被利用的保证（实测三个同分元素照抄原顺序）；多条件靠元组 key，而取负只对数字有效，字符串取负会抛 bad operand type for unary minus 那个 TypeError；reverse=True 把整个 key 的比较方向一起翻掉、每一位都跟着翻，而等键元素仍保持原顺序（这一点用三个同 key 的元素实测过）；key 函数在比较开始之前按原表顺序逐个调用（实测调用顺序与次数，以及为什么元组里混类型有时报有时不报）。另有 itemgetter 与 lambda 的实测速度差、cmp_to_key 返回的 KeyWrapper、字典四种 sorted 各排什么、tuple 没有 sort 方法，以及力扣 26 的写指针实现和它在空数组上的边界。"
tags: [Python, 学习笔记]
category: Python学习
...
因为 `reverse` 翻的是**比较的方向**，而比较是拿整个 key 元组一次做完的——它不知道元组里第一位是主条件、第二位是次条件，所以每一位跟着一起翻。

它翻的**不是**"排完之后把结果整体倒过来"。这两句话在大部分时候看不出区别，但等键元素能把它区分开：

```python
ties = [('a', 1), ('b', 1), ('c', 1)]
sorted(ties, key=lambda x: x[1], reverse=True)
```

```
等键 + reverse=True: [('a', 1), ('b', 1), ('c', 1)]    ← 还是 a→b→c
```

三个元素的 key 全是 `1`，如果真是"排完再整体倒序"，这里应该变成 `c→b→a`。**官方文档在这一点上写得很明白**：*"`reverse` 参数仍然保持排序稳定性"*——相等 key 的记录保留进入排序时的那个顺序。也就是说 `reverse=True` 下等键元素**照样保持原顺序**，这正是第八节"排两次"能配 `reverse` 用的前提。所以：
...
6. `reverse=True` 翻的是第一个条件、还是整个 key 的比较方向？等键元素会不会跟着倒序？怎么验？
...
- 因为比的是 key 而不是元素，所以 `reverse` 是把整个 key 的比较方向一起翻掉、看不见"哪个是主条件"（第五节）。
